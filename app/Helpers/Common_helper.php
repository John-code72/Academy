...
if (! function_exists('has_permission')) {
    function has_permission($route = '', $user_id = '')
    {
        ...
        $root_admin_id = App\Models\User::orderBy('id')->value('id');
        ...
        $raw = $get_admin_permission->permissions;
        // NULL / vide = pas encore configuré dans l'UI → même accès que l'admin principal
        if ($raw === null || $raw === '') {
            return true;
        }
        ...
        // Tableau JSON vide = aucune permission explicite (toggles tous désactivés)
...
if (! function_exists('ai_assistant_name')) {
    function ai_assistant_name(): string
    {
        if (function_exists('get_settings')) {
            $fromSettings = trim((string) get_settings('ai_assistant_name'));
            if ($fromSettings !== '') {
                return $fromSettings;
            }
        }

        $name = trim((string) config('gemini.assistant_name', 'Léa'));

        return $name !== '' ? $name : 'Léa';
    }
}

if (! function_exists('heygen_is_configured')) {
    function heygen_is_configured(): bool
    {
        ...
        return !empty($key) && !str_contains((string) $key, 'xxx');
    }
}

if (! function_exists('lesson_media_public_url')) {
    function lesson_media_public_url(?string $path): ?string
    {
        if ($path === null || $path === '') {
            return null;
        }
