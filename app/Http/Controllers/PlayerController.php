...
class PlayerController extends Controller
{
    public function course_player(Request $request, $slug, $id = '')
    {
        ...
        $personalizer = app(AiCoursePersonalizer::class);
        if ($lesson) {
            if ($lesson->video_provider === 'heygen' && $lesson->video_status === 'pending') {
                $personalizer->refreshHeyGenStatusForMainLesson($lesson);
                $lesson->refresh();
            }
            if (($lesson->ai_lesson_format ?? '') === 'audio' && !$lesson->audio_url) {
                $personalizer->ensureMainLessonAudio($lesson, $course->language ?? 'English');
                $lesson->refresh();
            }
        }
        $page_data['lesson_details'] = $lesson ?? Lesson::where('id', $id)->firstOrNew();
    ...
    public function refreshLessonHeygen(int $lesson)
    {
        $record = Lesson::findOrFail($lesson);
        $personalizer = app(AiCoursePersonalizer::class);
        $personalizer->refreshHeyGenStatusForMainLesson($record);
        $record->refresh();

        return response()->json([
            'status' => $record->video_status,
            'ready' => $record->video_status === 'ready' && (bool) $record->lesson_src,
            'reload' => $record->video_status === 'ready' ? 1 : 0,
            'error' => $record->video_error,
        ]);
    }
