...
<script type="text/javascript">
    ...
    function initAjaxFormsInModal() {
        if (typeof jQuery === 'undefined' || !jQuery.fn.ajaxForm) {
            return;
        }
        jQuery('#ajaxModal .ajaxForm:not(.initialized)').each(function() {
            jQuery(this).ajaxForm({
                beforeSend: function() {
                    jQuery('#ajaxModal .modal-body').css('opacity', '0.6');
                },
                complete: function(xhr) {
                    jQuery('#ajaxModal .modal-body').css('opacity', '1');
                    setTimeout(function() {
                ...
                error: function() {
                    jQuery('#ajaxModal .modal-body').css('opacity', '1');
                    error('{{ get_phrase('Request failed. Please try again.') }}');
                }
    ...
    function ajaxModal(url, title, modalClasses = 'modal-md', animation = 'fade') {
        $('#ajaxModal .modal-dialog').removeClass('modal-sm modal-md modal-lg modal-xl modal-xxl modal-fullscreen');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                initAjaxFormsInModal();
            },
            error: function() {
                $('#ajaxModal .modal-body').html(
                    '<p class="text-danger p-3">{{ get_phrase('Could not load this form. Please refresh the page.') }}</p>'
                );
            }
        });
    }
    ...
    myModalEl.addEventListener('hidden.bs.modal', event => {
        $('#ajaxModal .ajaxForm').removeClass('initialized');
        $('#ajaxModal .modal-body').html(
            '<div class="w-100 text-center py-5"><div class="spinner-border my-5" role="status"><span class="visually-hidden"></span></div></div>'
        );
    })
