...
@section('content')
    ...
    @php
        $aiMainPending = \App\Models\Lesson::where('video_provider', 'heygen')->where('video_status', 'pending')->count();
        $aiPersonalizedPending = \App\Models\PersonalizedCourseLesson::where('video_provider', 'heygen')->where('video_status', 'pending')->count();
    @endphp
    <div class="row g-2 g-sm-3 my-1">
        <div class="col-12">
            <div class="ol-card border-primary">
                <div class="ol-card-body px-20px py-3 d-flex flex-wrap align-items-center justify-content-between gap-3">
                    <div>
                        <h5 class="title fs-16px mb-1">
                            <i class="fi-rr-magic-wand me-2"></i>{{ get_phrase('AI Studio') }}
                        </h5>
                        <p class="sub-title fs-13px mb-0 text-muted">
                            {{ get_phrase('HeyGen pending') }}: {{ $aiMainPending }} {{ get_phrase('main') }},
                            {{ $aiPersonalizedPending }} {{ get_phrase('personalized') }}
                        </p>
                    </div>
                    <a href="{{ route('admin.ai.studio') }}" class="btn ol-btn-primary ol-btn-sm">
                        {{ get_phrase('Open AI Studio') }}
                    </a>
                </div>
            </div>
        </div>
