@include('course_player.ai_lesson_media')

@php
    $heygenVideoSrc = null;
    if (
        $lesson_details->video_provider === 'heygen'
        && $lesson_details->video_status === 'ready'
        && $lesson_details->lesson_src
    ) {
        $heygenVideoSrc = lesson_media_public_url($lesson_details->lesson_src);
    }
@endphp
    ...
    <div class="course-video-area border-primary border">
        <div class="course-video-wrap">
            <video width="100%" height="680" id="player" playsinline controls>
                <source src="{{ $heygenVideoSrc }}" type="video/mp4">
            </video>
            @include('course_player.player_config')
        </div>
    </div>
@elseif (isset($lesson_details->lesson_type))
    ...
    @elseif($lesson_details->lesson_type == 'html5')
        <div class="course-video-area border-primary border">
            ...
            <div class="course-video-wrap">
                <video width="100%" height="680" id="player" playsinline controls>
                    <source class="remove_video_src" src="{{ lesson_media_public_url($lesson_details->lesson_src) ?? $lesson_details->lesson_src }}" type="video/mp4">
                    @if(file_exists('public/uploads/lesson_file/videos/'.$lesson_details->id.'.vtt'))
                        <track label="Caption" kind="subtitles" srclang="cp" src="{{asset('uploads/lesson_file/videos/'.$lesson_details->id.'.vtt')}}" default />
                    @endif
                </video>
    ...
    @elseif($lesson_details->lesson_type == 'practice_test')
        <div class="course-video-area border-primary pb-5">
